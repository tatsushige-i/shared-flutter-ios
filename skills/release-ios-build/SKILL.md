...
本スキルは **リリースカット済みのブランチ上で実行されること**を前提とする。bump・ビルド・審査提出の
対象コミットが、実際にリリースされる内容と一致することを保証するため。

1. `git status --porcelain` で未コミット変更を確認。変更があれば内容を表示し、続行可否をユーザーに確認する。
2. `git branch --show-current` で現在ブランチを確認し、**リリースカット済みのブランチ上か**を検証する。
   - **`release/*` ブランチ上**（`release/*` 運用を導入済みのアプリ）: リリースカット済みと見なし、
     このブランチ上でそのまま bump コミットを作成する（新規ブランチは切らない）。
...
- タグは ASC にアップロードしたビルドのコミットと対応させるのが目的。Step 2 でリリースカット済みブランチ（`release/*` または `main`）上での実行を保証しているため、ビルド対象コミットはマージ後に `main` へ載る内容と一致し、マージ後の `main` に切ったタグの provenance はそのまま保たれる。
